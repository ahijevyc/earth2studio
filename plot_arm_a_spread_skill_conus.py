"""Plot Arm A spread vs. skill (RMSE vs. IFS analysis truth), CONUS.

Same as plot_arm_a_spread_skill.py but for the CONUS region (lat 20-60N,
lon 220-300E, matching the HWT domain in ens_spread_skill.ipynb) instead
of Global.
"""
...
fig.suptitle(
    "Arm A: ensemble spread vs. skill (CONUS, 50 members, mean across 14 inits)",
    fontsize=12.5,
    color=TEXT_PRIMARY,
    y=0.98,
)
...
for ax, v in zip(axes.flat, VARIABLES):
    ...
    series = {
        "spread": spread_data[f"{v}_conus_mean"],
        "rmse": rmse_data[f"{v}_conus_rmse"],
    }
...
fig.savefig("arm_a_spread_skill_conus.png", dpi=150, facecolor=fig.get_facecolor())
print("Saved arm_a_spread_skill_conus.png")
